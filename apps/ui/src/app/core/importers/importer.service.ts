...
export class ImporterService {
    ...
    public async importCharacter(
        ...
        if (
            !result.image.original_url.toLocaleLowerCase().endsWith('blank.png')
        ) {
    ...
    public async importTeam(
        ...
        if (
            !result.image.originalUrl.toLocaleLowerCase().endsWith('blank.png')
        ) {
            progress?.(`Fetching team image`);
            const response = await fetch(result.image.originalUrl);
            image = await response.blob();
        }
